  ...
  fetch(fileUrl, { method: 'HEAD' })   // check if URL exists ONLY works with internet connection
    .then(function(response) {
      if (response.ok) {
        ...
        }, 7000);  // this is the time needed for the sound effcet
...
document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('player').style.marginTop = 1000 + "px";
  document.getElementById('player').style.marginLeft = 2000 + "px";
});
function detectKey(e) {
  ...
  document.getElementById('player').style.marginLeft  = posLeft + "px";

  let playerPosition = { top: posTop, left: posLeft};
...
function checkPlayerPosition(playerPosition) {
  ...
  // Check if Player has reached the desired location
  if ( 700 <= playerPosition.top && playerPosition.top <= 750 && playerPosition.left <= 1425 && playerPosition.left >= 1375 ) {
    // Player has reached the desired location
    console.log('Player has reached the desired location!');
    var overlay = document.getElementById("overlay");
    ...
    var overlay = document.getElementById("overlay");
    var imageoverlay = document.getElementsByClassName("image-overlay")[0];
    imageoverlay.style.backgroundImage  = "url('media/box_overlay.png')";
